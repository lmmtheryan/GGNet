...
cmd:option('--unitPixels', 29, "the locator unit (1,1) maps to pixels (13,13), or (-1,-1) maps to (-13,-13)")
cmd:option('--locatorStd', 0.05, 'stdev of gaussian location sampler (between 0 and 1) (low values may cause NaNs)')
...
--[[data]]--
ds = torch.checkpoint(
   paths.concat(dp.DATA_DIR, 'checkpoint/dp.CTmnist60.t7'),
   function() return dp['CTmnist60']() end,
   opt.overwrite
)
...
VRClassReward = nn.GGClassReward0402(network, opt.rewardScale, opt.glimpsePatchSize, glanceNetwork, attention, ds:imageSize("h"))
