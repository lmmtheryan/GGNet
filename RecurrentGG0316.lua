...
------------------------------------------------------------------------
local RecurrentGG0316, parent = torch.class("nn.RecurrentGG0316", "nn.AbstractSequencer")

function RecurrentGG0316:__init(rnn, action, nStep, hiddenSize, glance, unitPixels)
   parent.__init(self)
...
function RecurrentGG0316:updateOutput(input)
   self.rnn:forget()
   self.action:forget()
   self.inputImages = input
...
function RecurrentGG0316:updateGradInput(input, gradOutput)
   ...
   for step=self.nStep,1,-1 do
      ...
      if step == 1 then
        ...
        --  print('updateGradInput',self._initInput:norm())
...
function RecurrentGG0316:accGradParameters(input, gradOutput, scale)
   assert(self.rnn.step - 1 == self.nStep, "inconsistent rnn steps")
   assert(torch.type(gradOutput) == 'table', "expecting gradOutput table")
   assert(#gradOutput == self.nStep, "gradOutput should have nStep elements")
   ...
   for step=self.nStep,1,-1 do
      ...
      if step == 1 then
         -- backward through initial starting actions
        --  print('accGradParameters',self._initInput:norm())
...
function RecurrentGG0316:accUpdateGradParameters(input, gradOutput, lr)
   assert(self.rnn.step - 1 == self.nStep, "inconsistent rnn steps")
   assert(torch.type(gradOutput) == 'table', "expecting gradOutput table")
   assert(#gradOutput == self.nStep, "gradOutput should have nStep elements")
   ...
   for step=self.nStep,1,-1 do
      ...
      if step == 1 then
         -- backward through initial starting actions
        --  print('accUpdateGradParameters',self._initInput:norm())
        --  assert(1==2)
...
function RecurrentGG0316:type(type)
   self._input = nil
   self._actions = nil
   self._crop = nil
   self._pad = nil
...
function RecurrentGG0316:__tostring__()
   local tab = '  '
   local line = '\n'
   local ext = '  |    '
   local extlast = '       '
